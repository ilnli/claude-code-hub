...
BEGIN
  ...
  IF COALESCE(last_status_code, status_code) BETWEEN 200 AND 299
     OR (
       COALESCE(last_status_code, status_code) IS NULL
       AND last_reason IN ('request_success', 'retry_success', 'hedge_winner')
     ) THEN
    RETURN 'success';
  END IF;
...
BEGIN
  ...
  IF NEW.blocked_by = 'warmup' THEN
    ...
    SET blocked_by = 'warmup',
        ...
        public_error_code = NEW.public_error_code
